                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.vehicle-allocations.update", [$vehicleAllocation->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('vehicle') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_id" id="vehicle_id" required>
                                @foreach($vehicles as $id => $vehicle)
                                    <option value="{{ $id }}" {{ (old('vehicle_id') ? old('vehicle_id') : $vehicleAllocation->vehicle->id ?? '') == $id ? 'selected' : '' }}>{{ $vehicle }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('region') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="region_id" id="region_id" required>
                                @foreach($regions as $id => $region)
                                    <option value="{{ $id }}" {{ (old('region_id') ? old('region_id') : $vehicleAllocation->region->id ?? '') == $id ? 'selected' : '' }}>{{ $region }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('division') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="division_id" id="division_id" required>
                                @foreach($divisions as $id => $division)
                                    <option value="{{ $id }}" {{ (old('division_id') ? old('division_id') : $vehicleAllocation->division->id ?? '') == $id ? 'selected' : '' }}>{{ $division }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('allottee_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="allottee_name_id" id="allottee_name_id" required>
                                @foreach($allottee_names as $id => $allottee_name)
                                    <option value="{{ $id }}" {{ (old('allottee_name_id') ? old('allottee_name_id') : $vehicleAllocation->allottee_name->id ?? '') == $id ? 'selected' : '' }}>{{ $allottee_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('allottee_designation') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="allottee_designation_id" id="allottee_designation_id" required>
                                @foreach($allottee_designations as $id => $allottee_designation)
                                    <option value="{{ $id }}" {{ (old('allottee_designation_id') ? old('allottee_designation_id') : $vehicleAllocation->allottee_designation->id ?? '') == $id ? 'selected' : '' }}>{{ $allottee_designation }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('allotment_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="allotment_date" id="allotment_date" value="{{ old('allotment_date', $vehicleAllocation->allotment_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $vehicleAllocation->description) !!}</textarea>
