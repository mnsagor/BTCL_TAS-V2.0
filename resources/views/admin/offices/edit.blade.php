                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.offices.update", [$office->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('region') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="region_id" id="region_id" required>
                                @foreach($regions as $id => $region)
                                    <option value="{{ $id }}" {{ (old('region_id') ? old('region_id') : $office->region->id ?? '') == $id ? 'selected' : '' }}>{{ $region }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $office->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('is_active') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Office::IS_ACTIVE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $office->is_active) === (string) $key ? 'checked' : '' }}>
