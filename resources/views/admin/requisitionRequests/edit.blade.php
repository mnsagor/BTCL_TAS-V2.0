        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.requisitionRequest.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.requisition-requests.update", [$requisitionRequest->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.requisitionRequest.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $requisitionRequest->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('designation') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="designation" id="designation" value="{{ old('designation', $requisitionRequest->designation) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('phone_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $requisitionRequest->phone_number) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('start_time') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="start_time" id="start_time" value="{{ old('start_time', $requisitionRequest->start_time) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('end_time') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="end_time" id="end_time" value="{{ old('end_time', $requisitionRequest->end_time) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('purpose') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="purpose" id="purpose">{!! old('purpose', $requisitionRequest->purpose) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('request_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="request_status" id="request_status" required>
                                ...
                                @foreach(App\Models\RequisitionRequest::REQUEST_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('request_status', $requisitionRequest->request_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
