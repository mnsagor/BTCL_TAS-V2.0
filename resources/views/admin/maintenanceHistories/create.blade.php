        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.maintenanceHistory.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.maintenance-histories.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('region_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="region_name_id" id="region_name_id" required>
                                @foreach($region_names as $id => $region_name)
                                    <option value="{{ $id }}" {{ old('region_name_id') == $id ? 'selected' : '' }}>{{ $region_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('work_detail') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="work_detail" id="work_detail">{!! old('work_detail') !!}</textarea>
