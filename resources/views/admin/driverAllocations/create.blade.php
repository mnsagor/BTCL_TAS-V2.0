        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.driverAllocation.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.driver-allocations.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('office_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="office_name_id" id="office_name_id" required>
                                @foreach($office_names as $id => $office_name)
                                    <option value="{{ $id }}" {{ old('office_name_id') == $id ? 'selected' : '' }}>{{ $office_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('driver_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_name_id" id="driver_name_id" required>
                                @foreach($driver_names as $id => $driver_name)
                                    <option value="{{ $id }}" {{ old('driver_name_id') == $id ? 'selected' : '' }}>{{ $driver_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('registration_number') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="registration_number_id" id="registration_number_id" required>
                                @foreach($registration_numbers as $id => $registration_number)
                                    <option value="{{ $id }}" {{ old('registration_number_id') == $id ? 'selected' : '' }}>{{ $registration_number }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('allocation_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="allocation_date" id="allocation_date" value="{{ old('allocation_date') }}" required>
